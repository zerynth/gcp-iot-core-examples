...
class GcpGui(tk.Frame):
    ...
    def subscription_callback(self, message):
        ...
        sample_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        self.new_messages_queue.put([sample_time, sample_values])
    ...
    def on_update(self):
        while True:
            try:
                message = self.new_messages_queue.get_nowait()

                sample_time, sample_values = message[0], message[1]
